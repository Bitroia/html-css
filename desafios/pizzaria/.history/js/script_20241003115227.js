...
function rolarParaCard(elementoAlvo, duracao) {
    const posicaoAlvo = elementoAlvo.getBoundingClientRect().top + window.pageYOffset; // Posição do alvo
    const posicaoInicial = window.pageYOffset;
    const distancia = posicaoAlvo - posicaoInicial; //Distancia entre a posicao atual e o alvo
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function(e){
        ...
        rolarParaCard(elementoAlvo, 400); //Duração da rolagem em milissegundos (400ms)
    });
});
